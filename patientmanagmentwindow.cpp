    ...
    submit(false) // Initialize submit as false
{
    ...
    ui->labelHello_2->setText("Hello "+currentPatient.name);
...
{
    hide();
    BookAppointmentWindow* book = new BookAppointmentWindow(this,currentPatient);
    book->show();
...
void PatientManagmentWindow::on_pushButtonSubmit_clicked()
{
    ...
    if (ui->comboBoxReschedule->isVisible()) {
        ...
        if (currentIndex >= 0) {
            ...
            BookAppointmentWindow* app = new BookAppointmentWindow(this,currentPatient);
            hide();
            app->show();
